...
function Tag({ label }) {
  return (
    <span className="px-2.5 py-1 rounded-md text-xs font-mono font-medium bg-gray-900 text-gray-400 border border-gray-800">
      {label}
    </span>
  )
}
...
export default function ProjectCard({ project }) {
  ...
  return (
    <article className="group relative flex flex-col gap-4 p-6 rounded-xl border border-gray-800 bg-gray-950 text-gray-300 hover:border-red-500/60 transition-all duration-300">

      <span
        aria-hidden="true"
        className="absolute inset-x-0 top-0 h-px rounded-full bg-gradient-to-r from-transparent via-red-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"
      />

      <header className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          <h3 className="text-base font-semibold text-white group-hover:text-red-400 transition-colors duration-200">
            {title}
          </h3>
          {featured && (
            <span className="px-1.5 py-0.5 rounded text-xs font-bold uppercase tracking-widest bg-red-950 text-red-400 border border-red-900">
              Destacado
            </span>
          )}
        </div>
      ...
      <footer className="flex items-center gap-2 pt-2 border-t border-gray-800">
        ...
        {!github && !demo && (
          <span className="text-xs text-gray-700 italic">Sin enlaces aun</span>
        )}
      </footer>
